...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { name, note, favorite } = await req.json();
    ...
    if (!existingNote) return apiResponse(false, "Note not found", 404);

    const updateNotes: Record<string, unknown> = {};
    if (name) updateNotes.name = name;
    if (note) {
      updateNotes.notes = existingNote.encrypt
        ? CryptoJS.AES.encrypt(note, vaultPin).toString()
        : note;
    }
    if (favorite === true || favorite === false) updateNotes.favorite = favorite;
